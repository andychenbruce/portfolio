...
function Boltzman(props) {
  ...
  return (
    <Wrapper head={new_head} {...props}>
      ...
      <br />
      diffuse color = <Tex texContent={"color_{diffuse}"} /> = (r', g', b') =
      (r, g, b)*lerp(
      <Tex texContent={"a"} />, <Tex texContent={"b"} />,{" "}
      <Tex texContent={"cos(\\theta)"} />)
      <br />
      ...
      <Tex texContent={"\\vec{l}"} /> and <Tex texContent={"\\vec{n}"} />, where
      the equation for reflection is{" "}
      <Tex texContent={"l_{ref} = l-{2l\\cdotp n \\over |n|^2}n"} />, then to
      find <Tex texContent={"cos(\\theta_{ref})"} /> just use the same dot
      product proof, then also clamp it between 0 and 1 again so that it avoid
      negative values
      <br />
      <br />
      the actual color value is based on a power of{" "}
      <Tex texContent={"cos(\\theta_{ref})"} />, the power being the shinyness
      <br />
      specular factor = <Tex texContent={"f = cos(\\theta_{ref})^s"} />, where{" "}
      <Tex texContent={"s"} /> is shinyness, and then the actual color is lerp(
      <Tex texContent={"color_{diffuse}"} />,{" "}
      <Tex texContent={"color_{light}"} />, <Tex texContent={"f"} />
      ), which is the equivelent of
      <br />
      final color ={" "}
      <Tex texContent={"f*color_{light} + (1-f)*color_{diffuse}"} />
      <br />
      this means that when <Tex texContent={"f"} /> is big or when{" "}
      <Tex texContent={"cos(\\theta_{ref})"} /> is near, meaning the angle is
      small the color is close to the light since it is reflecting into the
      camrea, and when <Tex texContent={"cos(\\theta_{ref})"} /> is small or the

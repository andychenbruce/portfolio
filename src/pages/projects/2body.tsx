...
function Project2Body(props) {
  ...
  return (
    <Wrapper head={new_head} {...props}>
      ...
      <p>Orbital equation where one body's mass is negligible <a href="https://en.wikipedia.org/wiki/Orbit_equation">wikipedia</a>
      </p>

      <div style={{textAlign: "center"}}>
	<Tex texContent={"r = {l^2 \\over m^2\\mu}{1 \\over 1+e \\cdot cos(\\theta)}"} />
      </div>
      Where<br/>


      <Tex texContent={"l"} /> = angular momentum<br/>
      <Tex texContent={"m"} /> = small body mass<br/>
      <Tex texContent={"M"} /> = large body mass<br/>
      <Tex texContent={"\\mu = GM"} /> = the standard gravitational parameter<br/>
      <Tex texContent={"e"} /> = the eccentricity of the orbit<br/>
      <Tex texContent={"\\theta"} /> = angle from the periapsis, the closest point<br/>

      <br/>
      <br/>
      
      If <Tex texContent={"l"} /> is the angular momentum, dividing it by the mass, <Tex texContent={"m"} />, would give the angular velocity, let that be <Tex texContent={"a"} />.
      ...
      <p>According to the <a href="https://en.wikipedia.org/wiki/Angular_momentum#Orbital_angular_momentum_in_three_dimensions">wikipedia page</a> for angular momentum it can be found that it is equal to the length of the cross product of the difference in position and the difference in velocity
      </p>

      <br/>
      let <Tex texContent={"\\vec{p}"} /> be the difference in postion of the 2 objects
      <br/>
      let <Tex texContent={"\\vec{v}"} /> be the difference in velocity of the 2 objects
      <br/>
      so then
      <br/>
      ...
      <p>According to this <a href="https://en.wikipedia.org/wiki/Eccentricity_vector">wikipedia page</a>, one can find an obscure thing called an eccentricity vector. It is a vector pointing from the apoapsis of the orbit to the periapsis with the magnitude of the eccentricity. It is calculated by
      </p>

      <br/>

      <Tex texContent={"\\vec{e} = {\\vec{v} \\times (\\vec{p} \\times \\vec{v}) \\over \\mu} - {\\vec{p} \\over ||\\vec{p}||}"} />
      ...
      <br/>
      <Tex texContent={"\\vec{e}"} /> goes from the apoapsis to the periapsis. Because the periapsis and apoapsis are always 180 degrees apart, it goes through the center so it also goes from the center to the periapsis. By rotating <Tex texContent={"\\vec{e}"} /> around the normal vector to the plane by <Tex texContent={"\\theta"} /> it will give the direction the position is in 3d space since <Tex texContent={"\\theta"} /> is defined as the angle from the periapsis.
      <br/>
      <br/>
      ...
      <br/>
      <br/>
      <p> <Tex texContent={"\\vec{e}"} /> can be rotated around <Tex texContent={"\\vec{n}"} /> using the <a href="https://en.wikipedia.org/wiki/Rodrigues%27_rotation_formula">Rodrigues' rotation formula</a>, which rotates a vector around a unit normal vector. Because the ecentricity vector also lies on the plane, <Tex texContent={"\\vec{n}"} /> is already a normal vector to it, it just needs to be normalised to a unit vector</p>
      <br/>
      <br/>
      <Tex texContent={"\\vec{m} = {\\vec{n} \\over ||\\vec{n}||}"} />
      <br/>
      <Tex texContent={"\\vec{o} = \\vec{e}cos(\\theta) + (\\vec{m} \\times \\vec{e})sin(\\theta) + \\vec{m}(\\vec{m} \\cdot \\vec{e})(1 - cos(\\theta))"} />
      <br/>
      <br/>

      Where <Tex texContent={"\\vec{o}"} /> would be pointing in the direction from the center to the position at angle $\theta$ from the periapsis.
      <br/>
      <br/>
      Now taking <Tex texContent={"\\vec{o}"} />, normalizing it into a unit vector, then multiplying by r will give the final position.
      <br/>
      <br/>
      ...
      <br/>
      <div style={{textAlign: "center"}}>
	<Tex texContent={"{a^2 \\over \\mu}{1 \\over 1+ecos(\\theta)}{\\vec{o} \\over ||\\vec{o}||}"} />
      </div>
      <br/>
      where
      <br/>
      ...
      <Tex texContent={"e = ||\\vec{e}||"} />
      <br/>
      <Tex texContent={"\\vec{n} = \\vec{p} \\times \\vec{v}"} /> or <Tex texContent={"\\vec{v} \\times \\vec{p}"} />, doesn't matter
      <br/>
      <Tex texContent={"\\vec{m} = {\\vec{n} \\over ||\\vec{n}||}"} />
      <br/>
      <Tex texContent={"\\vec{o} = \\vec{e}cos(\\theta) + (\\vec{m} \\times \\vec{e})sin(\\theta) + \\vec{m}(\\vec{m} \\cdot \\vec{e})(1 - cos(\\theta))"} />
      ...
      <br/>
      <p>The black lines are actually drawn pretty inefficently. Since the function is based on <Tex texContent={"\\theta"} /> which is an angle the farther away portions of orbits get only a few points since they occupy less of the angle, you can see the far out parts of the elipses get jankey since they are drawn with fewer points. Also the lines are draw inefficiently as they are buffered into the GPU. The lines share their start and end points with each other, but when drawing it duplicates points in the buffer and then passes it to the GPU. The data passed to the GPU every frame could be basically halved in size by using indices, but for now it runs fine at least on my computer.</p>
      <hr/>
      <hr className="clearLeft" />

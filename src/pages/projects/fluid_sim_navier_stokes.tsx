...
function NavierStokes(props) {
  ...
  let new_heads = (<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.12.0/dist/katex.min.css" integrity="sha384-AfEj0r4/OFrOo5t7NnNe46zW/tFgW6x/bCJG8FqQCEo3+Aro6EYUG4+cU+KJWu/X" crossOrigin="anonymous" />);
  ...
  return (
	...
	<Tex texContent={"\\rho{D\\mathbf{u} \\over Dt} = -\\nabla p + \\nabla \\cdotp \\mathbf{ \\tau } + \\rho \\mathbf{g}"} />
      </div>
      <p>where</p>
      <br/>
      <p><Tex texContent={"\\rho"} /> is density</p>
      <br/>
      ...
      <br/>
      <Tex texContent={"g"} /> is any other acceleration, basically the "other" category like walls, gravity, surface tension, diffusion, ect
      <br/>
      <Tex texContent={"\\nabla"} /> is the gradient, equal to <Tex texContent={"({\\partial \\over \\partial x}, {\\partial \\over \\partial y}, {\\partial \\over \\partial z}, ...)"} /> for however many dimentions. 
      This simulation is only 2 dimentional for display purpouses so in this case it is just <Tex texContent={"({\\partial \\over \\partial x}, {\\partial \\over \\partial y})"} />.
											       <br/>
      <Tex texContent={"{\\nabla \\cdotp}"} /> is the divergence and is just the dot product of the gradient with the origional vector so it is equal to to<Tex texContent={"({\\partial \\over \\partial x}, {\\partial \\over \\partial y}, {\\partial \\over \\partial z} ...) \\cdotp (F_x, F_y, F_z, ...) = {\\partial F_x\\over \\partial x} + {\\partial F_y\\over \\partial y} + {\\partial F_z\\over \\partial z}, ..."} /> for however many dimentions. Again this one is 2 dimentions so it's just<Tex texContent={"{\\partial F_x\\over \\partial x} + {\\partial F_y\\over \\partial y}"} />.
      <br/>
      <Tex texContent={"D\\mathbf{} \\over Dt"} /> is the material vector which is just <Tex texContent={"{\\partial  \\over \\partial t} + \\mathbf{u} \\cdotp \\nabla"} />, basically meaning how a material follows its velocity field
      <br/>
      here it is taking the matieral derivative of the velocity, or saying how the velocity follows itself

      <br/>
      <Tex texContent={"{D\\mathbf{u} \\over Dt} = {\\partial\\mathbf{u} \\over \\partial t} + \\mathbf{u} \\cdotp \\nabla \\mathbf{u}"} />
      <br/>
      which expanded further makes
      <br/>
      <Tex texContent={"{\\partial\\mathbf{u} \\over \\partial t} + {u_x}{\\partial\\mathbf{u} \\over \\partial x} + {u_y}{\\partial\\mathbf{u} \\over \\partial y}"} />
      <br/>
      which is actually just equal to 
      <br/>
      <Tex texContent={"d\\mathbf{u} \\over dt"} /> which the acceleration vector, so the velocity moving based on the velocity field (iteself) is just the acceleration field
      <br/>
      let<Tex texContent={"\\mathbf{a}"} /> be the acceleration vector
      <br/>
	...
	<Tex texContent={"\\rho{\\mathbf{a}} = -\\nabla p + \\nabla \\cdotp \\mathbf{ \\tau } + \\rho \\mathbf{g}"} />
      ...
      next,
      <br/>
      <Tex texContent={"\\mathbf{\\tau}"} /> basically doesn't matter here since it is the stress tensor used for viscosity and when the shape of the box is changing size and volume, stuff like that is ignored for this simulation
      <br/>
	...
	<Tex texContent={"\\rho{\\mathbf{a}} = -\\nabla p + \\rho \\mathbf{g}"} />
      ...
      <br/>
      <div style={{textAlign: "center"}}><Tex texContent={"\\rho{\\mathbf{a}} = -\\nabla p"} /></div>
      <br/>
      ...
      <br/>
      In incompressable fluids, density is constant so<Tex texContent={"\\rho"} /> is basically a constant and can be ignored too. Because<Tex texContent={"\\rho"} /> is constant,<Tex texContent={"{d\\rho \\over dt} = 0"} />
      <br/>
      conservation of mass equation is
      <div style={{textAlign: "center"}}><Tex texContent={"{d\\rho \\over dt} + \\nabla \\cdotp (\\rho \\mathbf{u}) = 0"} /></div>
      <br/>
      but because<Tex texContent={"{d\\rho \\over dt} = 0"} /> it is just<Tex texContent={"\\nabla \\cdotp (\\rho \\mathbf{u}) = 0"} />, and since<Tex texContent={"\\rho"} /> doesnt change it can be divided out of the divergence since it is the same at every point to where<Tex texContent={"\\nabla \\cdotp \\mathbf{u} = 0"} />, which also makes sense. One point can't have 2 neighboring points both pulling from it or else that means it is going in 2 directions and creating new mass, or if a cell has 2 neighbors going into it mass is deleted since density can't change, so the divergence at every point must equal zero.
      <br/>
      The opposite of divergence is curl, and every vector field can be reresented as a vector field of just the divergence plus a field of just the curl,<Tex texContent={"\\mathbf{u_{divergence}} + \\mathbf{u_{curl}} = \\mathbf{u}"} />. To get just the curl one takes the origional vector field and subtracts the divergence to leave the curl,<Tex texContent={"\\mathbf{u_{curl}} = \\mathbf{u} - \\mathbf{u_{divergence}}"} />. The divergence only field,<Tex texContent={"\\mathbf{u_{divergence}}"} /> is really complicated to calculate and would take like 10 pages to explain but basically it's just:
      <br/>
      <Tex texContent={"p(x, y) = {p(x-1, y) + (p(x+1, y) + p(x, y-1) + p(x, y+1) -  {\\nabla \\cdotp \\mathbf{u}(x, y)} \\over 4}"} />
      <br/>
      ...
      <br/>
      to then calculate<Tex texContent={"\\mathbf{u_c} = \\mathbf{u} - \\mathbf{u_d}"} /> and then use<Tex texContent={"\\mathbf{u_c}"} /> as the new<Tex texContent={"\\mathbf{u}"} /> for the next time step
																	   <br/>
      <br/>
      In the code this systems of equations could be solved with a massive matrix with each voxel as a variable and have very perfect calculations, or just do it iteratevly and approximate with an iterative solver. This code uses the <a href="https://en.wikipedia.org/wiki/Gauss%E2%80%93Seidel_method">Gauss Seidel</a> method, basically just putting random numbers in to start, then going through each<Tex texContent={"p"} />-value and substituting the other<Tex texContent={"p"} />-values over and over and hope it converges to the right solution (I'm not even joking this is literatly what the code does).
      <br/>
      ...
      <br/>
      <div style={{textAlign: "center"}}>
	<Tex texContent={"a_n(x, y) = a_c(x, y) + k*({a_n(x-1, y) + a_n(x+1, y) + a_n(x, y-1) + a_n(x, y+1)\\over 4} - a_c(x, y))"} />
      </div>
      <br/>
      between the new and the current attrubute,<Tex texContent={"a"} /> and the percent it goes towards it in that time tick,<Tex texContent={"k"} />
      <br/>
      ...
      <br/>
      <Tex texContent={"{df(x) \\over dx} = lim_{h \\to 0}{f(x+h)-f(x) \\over h}"} />
      <br/>
      <br/>
      which can also be
      <br/>
      <br/>
      <Tex texContent={"{df(x) \\over dx} = lim_{h \\to 0}{f(x+h)-f(x-0) \\over h-(-0)}"} />
      <br/>
      <br/>
      then to
      <br/>
      <br/>
      <Tex texContent={"{df(x) \\over dx} = lim_{h \\to 0}{f(x+h)-f(x-h) \\over h-(-h)}"} />
      <br/>
      <br/>
      <Tex texContent={"{df(x) \\over dx} = lim_{h \\to 0}{f(x+h)-f(x-h) \\over 2h}"} />
      <br/>
      <br/>

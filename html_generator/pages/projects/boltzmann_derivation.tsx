...
function DeltaGcode({title}: {title: string}) {
  return (
    <Wrapper title={title}>
      ...
      <MakeMathDisplay tex={"\\ln(p(r_\\text{sys})) = \\ln(W_\\text{bath}(E_\\text{total})) - \\frac{d\\ln(W_\\text{bath}(E_\\text{total}))}{dE_\\text{total}}E_\\text{sys}(r_\\text{sys})"} />
      <MakeMathDisplay tex={"+ \\frac{1}{2}\\frac{d^2\\ln(W_\\text{bath}(E_\\text{total}))}{dE_\\text{total}^2}E_\\text{sys}(r_\\text{sys})^2 - \\frac{1}{6}\\frac{d^3\\ln(W_\\text{bath}(E_\\text{total}))}{dE_\\text{total}^3}E_\\text{sys}(r_\\text{sys})^3 + \\cdots"} />
      <MakeMathDisplay tex={"\\cdots - \\ln(W_{\\text{total}}(E_{\\text{total}}))"} />
      <p>In the limit that <MakeMath tex={"E_\\text{total}"} /> approaches <MakeMath tex={"E_\\text{bath}"} /> the bath becomes a microcanonical ensemble and the entropy of the bath <MakeMath tex={"S_\\text{bath} (E_\\text{total}) = S_\\text{bath} (E_\\text{bath}) = k_B\\ln(W_\\text{bath}(E_\\text{bath}))"} /></p>
      <MakeMathDisplay tex={"= \\ln(W_\\text{bath}(E_\\text{bath})) - \\frac{1}{k_B}\\frac{dS_\\text{bath}(E_\\text{bath})}{dE_\\text{bath}}E_\\text{sys}(r_\\text{sys}) + \\frac{1}{2k_B}\\frac{d^2S_\\text{bath}(E_\\text{bath})}{dE_\\text{bath}^2}E_\\text{sys}(r_\\text{sys})^2"} />
      <MakeMathDisplay tex={"- \\frac{1}{6k_B}\\frac{d^3S_\\text{bath}(E_\\text{bath})}{dE_\\text{bath}^3}E_\\text{sys}(r_\\text{sys})^3 + \\cdots - \\ln(W_{\\text{total}}(E_{\\text{total}}))"} />
      <p>And by definition <MakeMath tex={"\\frac{1}{T_\\text{bath}} = \\frac{dS_\\text{bath}(E)}{dE}"} />.</p>
      <MakeMathDisplay tex={"= \\ln(W_\\text{bath}(E_\\text{bath})) - \\frac{1}{k_B T_\\text{bath}}E_\\text{sys}(r_\\text{sys}) + \\frac{1}{2k_B T_\\text{bath}}\\frac{dT_\\text{bath}}{dE_\\text{bath}^2}E_\\text{sys}(r_\\text{sys})^2"} />
      <MakeMathDisplay tex={"- \\frac{1}{6k_B T_\\text{bath}}\\frac{d^2T_\\text{bath}}{dE_\\text{bath}^2}E_\\text{sys}(r_\\text{sys})^3 + \\cdots - \\ln(W_{\\text{total}}(E_{\\text{total}}))"} />
      ...
      <MakeMathDisplay tex={"\\ln(p(r_\\text{sys})) = - \\frac{1}{k_B T_\\text{bath}}E_\\text{sys}(r_\\text{sys}) + \\ln \\Big(\\frac{W_\\text{bath}(E_\\text{bath})}{W_{\\text{total}}(E_{\\text{total}})} \\Big)"} />
      <p>Then exponentiate both sides.</p>
      <MakeMathDisplay tex={"p(r_\\text{sys})= \\frac{e^{-\\frac{E_\\text{sys}(r_\\text{sys})}{k_B T_\\text{bath}}}}{\\Big(\\frac{W_\\text{total}(E_\\text{total})}{W_{\\text{bath}}(E_{\\text{bath}})} \\Big)}"} />
      ...
      <p>Because you define <MakeMath tex={"S_\\text{max}"} /> with <MakeMath tex={"\\lambda_2"} /> but then define <MakeMath tex={"\\lambda_2"} /> as the derivative of <MakeMath tex={"S_\\text{max}"} /> which creates a circular definition. For this part to be valid you must already know either the temperature (canonical ensemble) or the average energy.</p>
      <hr/>
